...
** Environment
The project is packaged with [[https://docs.astral.sh/uv/][uv]] and targets modern PyTorch (2.x). Create the
...
This installs =torch=, =torchvision=, =numpy=, =scipy=, =tqdm=, =pyyaml=,
=matplotlib=, =hydra-core= and the =hsicbt= package itself (see =pyproject.toml=).
Datasets (MNIST / FashionMNIST / CIFAR-10) are downloaded automatically on first
use into =assets/data=.

** Experimenting
Experiments are configured with [[https://hydra.cc][Hydra]] and run through two console scripts:

#+begin_src bash
# train (any config key can be overridden on the command line)
uv run hsicbt experiment=general-hsicbt epochs=5 learning_rate=0.001

# plot the figures for a task
...
- Linting/formatting is handled by [[https://docs.astral.sh/ruff/][ruff]] (=uv run ruff check src=, =uv run ruff format src=).
