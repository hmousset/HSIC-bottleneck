...
from ..utils.path import get_act_path, get_exp_path, get_log_filepath, get_plot_filename
...
def plot_hsicsolve_result(config_dict):
    ...
    try:
        ...
        out_hsic_epoch = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_HSICTRAIN, config_dict["data_code"])
        )["epoch_log_dict"]
    except OSError as e:
        print_highlight(
    ...
    filepath = get_exp_path(
        "fig4-{}-test-acc.{}".format(get_plot_filename(config_dict), config_dict["ext"])
    )
    ...
    plot.plot_activation_distribution(
        get_act_path(config_dict["task"], config_dict["training_type"], config_dict["data_code"])
    )
    filepath = get_exp_path(
        "fig3-hsic-solve-actdist-{}.{}".format(config_dict["data_code"], config_dict["ext"])
    )
    save_experiment_fig(filepath)
