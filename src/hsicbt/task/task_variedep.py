...
def plot_variedep_result(config_dict):
    ...
    filepath = get_exp_path(
        "fig5a-varied-epoch-acc-{}.{}".format(config_dict["data_code"], config_dict["ext"])
    )
    ...
    filepath = get_exp_path(
        "fig5b-varied-epoch-loss-{}.{}".format(config_dict["data_code"], config_dict["ext"])
    )
    save_experiment_fig(filepath)
    ...
    plot.save_figure(
        get_exp_path(
            "fig5a-{}-epoch-test-acc.{}".format(get_plot_filename(config_dict), config_dict["ext"])
        )
    )
    ...
    plot.save_figure(
        get_exp_path(
            "fig5b-{}-epoch-test-loss.{}".format(get_plot_filename(config_dict), config_dict["ext"])
        )
    )
