...
from ..core.engine import training_format_combined
...
def plot_sigmacomb_result(config_dict):
    ...
    try:
        out_standard_batch_1 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 1)
        )["batch_log_list"]
        out_standard_batch_2 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 2)
        )["batch_log_list"]
        out_standard_batch_3 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 3)
        )["batch_log_list"]
        out_standard_batch_4 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 4)
        )["batch_log_list"]
        out_standard_epoch_1 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 1)
        )["epoch_log_dict"]
        out_standard_epoch_2 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 2)
        )["epoch_log_dict"]
        out_standard_epoch_3 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 3)
        )["epoch_log_dict"]
        out_standard_epoch_4 = load_logs(
            get_log_filepath(config_dict["task"], TTYPE_FORMAT, config_dict["data_code"], 4)
        )["epoch_log_dict"]
    except OSError as e:
        print_highlight(
    ...
    input_list = [
        out_standard_batch_1,
        out_standard_batch_2,
        out_standard_batch_3,
        out_standard_batch_4,
    ]
    label_list = [r"$\sigma$=1", r"$\sigma$=5", r"$\sigma$=10", r"$\sigma$-combined"]
    metadata = {
    ...
    filepath = get_exp_path(
        "fig6b-{}-sigmacomb-train-acc.{}".format(get_plot_filename(config_dict), config_dict["ext"])
    )
    save_experiment_fig(filepath)

    input_list = [
        out_standard_epoch_1,
        out_standard_epoch_2,
        out_standard_epoch_3,
        out_standard_epoch_4,
    ]
    label_list = [r"$\sigma$=1", r"$\sigma$=5", r"$\sigma$=10", r"$\sigma$-combined"]
    metadata = {
    ...
    filepath = get_exp_path(
        "fig6b-{}-sigmacomb-test-acc.{}".format(get_plot_filename(config_dict), config_dict["ext"])
    )
    save_experiment_fig(filepath)
...
def task_sigmacomb_func(config_dict):
    ...
    if isinstance(config_dict["exp_index"], list):
        model_list = [
            f"{os.path.splitext(model_filename)[0]}-{i:04d}.pt" for i in config_dict["exp_index"]
        ]
