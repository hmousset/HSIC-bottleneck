...
from ..utils import meter, misc
from .train_misc import hsic_objective
...
def hsic_train(cepoch, model, data_loader, config_dict):
    ...
    for batch_idx, (data, target) in pbar:
        ...
        for i in range(len(hiddens)):
            output, hiddens = model(data)
            params, param_names = misc.get_layer_parameters(
                model=model, idx_range=idx_range[i]
            )  # so we only optimize one layer at a time
            optimizer = optim.SGD(
                params, lr=config_dict["learning_rate"], momentum=0.9, weight_decay=0.001
            )
            optimizer.zero_grad()
        ...
        msg = "Train Epoch: {cepoch} [ {cidx:5d}/{tolidx:5d} ({perc:2d}%)] H_hx:{H_hx:.4f} H_hy:{H_hy:.4f}".format(
            ...
            tolidx=n_data,
            perc=int(100.0 * (batch_idx + 1) * config_dict["batch_size"] / n_data),
