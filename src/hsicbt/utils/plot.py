...
def plot_epoch_log(curve_list, ptype, metadata):
    ...
    for i, curve_dict in enumerate(curve_list):
        ...
        ax.plot(curve_dict[ptype], "*", ms=30, alpha=0.3, color=p[0].get_color())
    ...
    if abs(end_max_y - end_min_y) > abs(max_y - min_y) / 2.0:
        plt.legend(fontsize=FONTSIZE_LEDEND)
    else:
...
def plot_batches_log(curve_list, ptype, metadata):
    ...
    for i, curve_dict in enumerate(curve_list):
        ...
        ax.fill_between(np.arange(len(val)), v - e, v + e, color=p[0].get_color(), alpha=0.25)
...
def plot_activation_distribution(datapath, title):
    ...
    for i in range(10):
        ...
        num_correct = np.where(out == np.argmax(y))[
            0
        ]  # comparing how many samples match the maximum of mean
    ...
    for i in range(10):
        ...
        out = np.array(
            [np.argmax(vec) for vec in select_item]
        )  # find the maximum arg of activation dist
        ...
        subplot.fill_between(
            np.arange(10), y - e, y + e, alpha=0.5, edgecolor="#CC4F1B", facecolor="#FF9848"
        )
    ...
    title += f"  class argmax {shuffled_list}"
...
def adding_footnote(fig, text):
    fig.text(0.0, 0.0, text, fontsize=FONTSIZE_FOOTNOTE)


def save_figure(filepath):
    plt.savefig(filepath, bbox_inches="tight")
    plt.clf()
    print_highlight(f"Saved   [{filepath}]", ctype="blue")
