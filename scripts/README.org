...
#+begin_src bash
# in bash
git clone https://github.com/choasma/HSIC-bottleneck.git
cd HSIC-bottleneck
uv sync                  # create the environment and install hsicbt
bash scripts/batch.sh    # reproduce every figure
#+end_src

After the batch training, you'll get the following result in your *assets/exp* folder, and please compare to our sample results under *assets/samples* folder:
For more information, please visit the configuration page ([[file:../config/README.org][config/README.org]]) and the entry-point page ([[file:../bin/README.md][bin/README.md]]).
#+begin_src bash
...
- create the environment once with *uv sync*
- run a script prefixed with *task_* (e.g. *bash scripts/task_general.sh*)
- it selects a Hydra experiment config *experiment=[name]*
- training,
